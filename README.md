...
>  1. **猫咪进入** -- 根据DPID 24中的`DEV_cat_enter 8 `来判断
>  2. **猫咪离开** -- 在猫咪进入的前提下,DPID 24中的`DEV_standby 0` 判断为猫咪离开,但是硬件建议不将`猫咪离开`写入日志,因为目前没有专门的DP ID来判断猫咪离开,为此下列日志内容同步去除`猫咪离开`的相关日志
